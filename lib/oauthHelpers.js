...
// Les snippets Gmail sont encodés HTML (&#39; &amp; …), parfois doublement
// (&amp;#160; chez certains expéditeurs) — on décode jusqu'à stabilité.
// L'échappement anti-XSS reste à la charge du frontend au moment du rendu.
...
function decodeEntities(str) {
  ...
  do { prev = out; out = decodeEntitiesOnce(prev); } while (out !== prev && ++guard < 3);
  return out;
}
  ...
  const emails = await Promise.all(ids.map(async ({ id }) => {
    try {
      ...
      return {
        ...
        subject:     decodeEntities(hdrs.subject) || '(sans objet)',
        date:        hdrs.date ? new Date(hdrs.date).toISOString() : new Date().toISOString(),
        body:        decodeEntities(msg.snippet || ''),
      };
    } catch { return null; }
  }));
...
// ── Envoi d'une réponse via l'API Gmail ──────────────────────
// PRÉPARÉ pour le scope gmail.send (feature flag ENABLE_SEND).
// Relit le message original pour répondre au vrai expéditeur (anti-IDOR)
// et conserver le fil (In-Reply-To / References / threadId).
async function sendGmailReply(accessToken, messageId, content) {
  const H = { Authorization: `Bearer ${accessToken}` };

  const orig = await (await fetch(
  ...
  const hdrs = Object.fromEntries((orig.payload?.headers || []).map(h => [h.name.toLowerCase(), h.value]));
  const toM  = (hdrs.from || '').match(/<([^>]+)>/);
  const to   = toM ? toM[1] : hdrs.from;
  if (!to) return null;

  const subject = /^re\s*:/i.test(hdrs.subject || '') ? hdrs.subject : `Re: ${hdrs.subject || ''}`;
  const raw = [
    `To: ${to}`,
    `Subject: =?UTF-8?B?${Buffer.from(subject).toString('base64')}?=`,
    hdrs['message-id'] ? `In-Reply-To: ${hdrs['message-id']}` : '',
    hdrs['message-id'] ? `References: ${hdrs['message-id']}`   : '',
  ...
  if (!res.ok) {
    ...
    console.error('[Gmail send]', res.status, err?.error?.message || '');
    return null; // scope manquant ou autre → le caller bascule sur mailto
  }
  return { to };
}

module.exports = { getValidAccessToken, fetchGmailEmails, fetchOutlookEmails, sendGmailReply };
