...
async function translateChunkMyMemory(chunk, from, to) {
  ...
  // Quota journalier épuisé → responseStatus 403 avec un texte d'avertissement
  if (!txt || data.responseStatus >= 400 || /MYMEMORY WARNING/i.test(txt)) {
    throw new Error('MyMemory : ' + (data?.responseDetails || 'quota ou réponse invalide'));
  }
  return txt;
}

// Moteur de secours gratuit : endpoint public Google Translate (sans clé).
// Le quota MyMemory est par IP — sur Vercel (IP partagées) il s'épuise vite.
async function translateChunkGtx(chunk, from, to) {
  ...
  const data = await res.json();
  const txt  = (data?.[0] || []).map(seg => seg?.[0] || '').join('');
  if (!txt) throw new Error('gtx : réponse vide');
  return txt;
}

async function translateChunk(chunk, from, to) {
  try { return await translateChunkMyMemory(chunk, from, to); }
  catch (e1) {
    try { return await translateChunkGtx(chunk, from, to); }
